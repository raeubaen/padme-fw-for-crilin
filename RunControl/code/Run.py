...
class Run:
    ...
    def __init__(self):
        ...
        self.multidaq_executable = "%s/PadmeMultiDAQ/PadmeDAQ.exe"%self.padme_fw
    ...
    def change_run(self):
        ...
        self.multidaq_log = "%s/%s_multidaq.log"%(self.log_dir,self.log_file_head)
    ...
    def read_setup(self):
        ...
        re_board_link = re.compile("^\s*(\d+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(\d+)\s*$")
        ...
        for l in f:
            ...
            if (m):
                ...
                if (p_name == "boardid_list"):
                    ...
                    if (mm):
                        self.board_link_list.append(mm.group(1,2,3,4,5))
                    else:
                ...
                elif (p_name == "multidaq"):
                    self.multidaq = p_value
                else:
    ...
    def config_list(self):
        ...
        cfg_list.append(["multidaq",          self.multidaq])
        if (self.multidaq == "ON"):
            cfg_list.append(["multidaq_executable",self.multidaq_executable])
            cfg_list.append(["multidaq_log",       self.multidaq_log])
        ...
        for b in self.boardid_list:
            ...
            for link in self.board_link_list:
                (board,host,mode,port,node) = link
                if b == int(board):
                    #board_link = "%s %s %s %s %s"%(board,host,mode,port,node)
                    cfg_list.append(["board_link","%s %s %s %s %s"%(board,host,mode,port,node)])
    ...
    def create_log_dir(self):
        ...
        if not os.path.exists(self.log_dir): os.makedirs(self.log_dir,0o755)

    def write_config(self):

        # Create config directory for this run (make sure the full tree is there)
        if not os.path.exists(self.config_dir): os.makedirs(self.config_dir,0o755)
    ...
    def create_fifos(self):
        ...
        if self.trigger.node_id == 0:
            if not os.path.exists(self.stream_dir): os.makedirs(self.stream_dir,0o755)
    ...
    def create_level1_output_dirs(self):
        ...
        for level1 in self.level1_list:
            ...
            if level1.node_id == 0:
                if not os.path.exists(level1.output_dir): os.makedirs(level1.output_dir,0o755)
            else:
    ...
    def change_setup(self,setup):
        ...
        for node_id in self.daq_nodes_id_list:
            ...
            self.daq_nodes_ip_list[node_id] = ""
